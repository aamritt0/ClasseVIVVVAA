...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      // Check if token is likely still valid (Classeviva tokens last a while but let's just try)
      setUser(parsedUser);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/classeviva/login', {
        method: 'POST',
  ...
  const fetchAllData = async (studentId: string, token: string) => {
    ...
    try {
      const [gradesRes, lessonsRes, absencesRes, periodsRes] = await Promise.all([
        fetch(`/api/classeviva/grades/${cleanId}`, { headers: { 'Z-Auth-Token': token } }),
        fetch(`/api/classeviva/proxy/${cleanId}/lessons/today`, { headers: { 'Z-Auth-Token': token } }),
        fetch(`/api/classeviva/proxy/${cleanId}/absences`, { headers: { 'Z-Auth-Token': token } }),
        fetch(`/api/classeviva/proxy/${cleanId}/periods`, { headers: { 'Z-Auth-Token': token } })
      ]);
      ...
      if (periodsRes.ok) {
        ...
        // Set default period to the current one if possible
        const now = new Date();
      ...
      <main className="max-w-2xl mx-auto p-4 space-y-6">
        ...
        {activeTab === 'grades' && (
          <div className="space-y-6">
            ...
            {/* Top Bar Chart representation (Overview) */}
              ...
              {lessons.length === 0 ? (
                ...
                >
                  <div className="flex gap-4 items-center w-full">
                    {/* Placeholder Time since API doesn't provide it simply without parsing */}
